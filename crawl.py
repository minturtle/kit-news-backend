        ...
        for url in target_urls:
            ...
            for content_url in post_urls:  # 기사 url
                ...
                try:
                    ...
                    # 기사 시간대 가져옴
                    article_time = document_content.find_all('span', {
                        'class': "media_end_head_info_datestamp_time _ARTICLE_DATE_TIME"})[0]['data-date-time']

                    # CSV 작성
                    cursor.execute(
                        "INSERT INTO news_data (article_datetime, article_category, media, article_title, article_body, article_address) VALUES (%s, %s, %s, %s, %s, %s)",
                        (article_time, category_name, text_company, text_headline, text_sentence, content_url))
                    cnx.commit()
                    print(category_name, ": ", cnt)
                    cnt +=1


                    if cnt >= 50:
                        print(f"Count out: Crawling for {category_name} sucess.")
                        cursor.close()
                        cnx.close()
                        return
...
if __name__ == "__main__":
    ...
    Crawler.set_category('정치', '경제', '사회', '생활문화', 'IT과학') # '세계'는 수집 속도가 너무 느려 일단 뺌
    today = datetime.now().__format__('%Y-%m-%d')
    Crawler.set_date_range(today, today) # ('2023-07-21', '2023-07-21')
    Crawler.start()